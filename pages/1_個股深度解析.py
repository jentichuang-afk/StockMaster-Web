                ...
                if submit_clicked and user_question.strip():
                    if not active_experts:
                        ...
                        for active_cfg in active_experts:
                            ...
                            # 找出這個專家在 original config 中的索引，以確定其特定人設
                            original_idx = 0
                            for o_idx, o_cfg in enumerate(st.session_state["expert_configs"]):
                                if o_cfg["name"] == expert_name:
                                    original_idx = o_idx
                                    break
                            
                            expert_personas = [
                                "巴菲特價值流派：非常注重基本面、本益比、護城河與安全邊際，語氣沉穩保守，講究長期價值與規避風險。",
                                "凱薩琳科技成長流派：極度熱愛破壞性創新、AI與未來大趨勢，語氣樂觀犀利，能承受高波動，熱衷尋找下一個引領未來的超級企業。",
                                "索羅斯總經投機流派：著重於全球資金流向、反射性理論、技術分析背離、量價結構與投機反轉點，語氣冷靜且投機，關注宏觀經濟與市場非理性帶來的交易機會。"
                            ]
                            active_persona = expert_personas[original_idx % 3]
                            ...
                            【重要任務與發言規則 - 攸關生死】：
                            1. 你【只能】代表你自己（{expert_name}）發言！你絕對不可以代表其他專家發言，也絕對不要在你的回答中模擬其他專家的對話（例如不要自己寫『凱薩琳說...』、『索羅斯說...』，你不是多人角色扮演劇本模擬器）。
                            2. 請嚴格根據上述你的流派人設，對使用者或前面專家提出的問題給出你單一專家的專業分析段落。
                            3. 討論串中可能包含其他專家的發言。你**絕對必須**看清前面的討論，並針對前面的論點進行有理有據的贊同、反駁或補充！
                            4. 請用精煉且有說服力的繁體中文回答，語氣要活生生像個獨立的專家。
                            """
                ...
                if free_submit_clicked and free_user_question.strip():
                    if not free_active_experts:
                        ...
                        for active_cfg in free_active_experts:
                            ...
                            system_instruction = f"""
                            你現在是投資大師「{expert_name}」。你目前正在參與一場自由漫聊會診，這場會診不限特定股票標的，大家正在探討更廣泛的財經與人生話題。
                            ...
                            【重要任務與發言規則 - 攸關生死】：
                            1. 你【只能】代表你自己（{expert_name}）發言！你絕對不可以代表其他專家發言，也絕對不要在你的回答中模擬其他專家的對話（例如不要自己寫『凱薩琳說...』、『索羅斯說...』，你不是多人角色扮演劇本模擬器）。
                            2. 請嚴格根據上述你的流派人設，對使用者或前面專家提出的問題給出你單一專家的專業分析段落。回答時緊扣使用者提出的宏觀、趨勢、理財、哲學或其他主題，不需要強行套用特定股票的最新數據。
                            3. 討論串中可能包含其他專家的發言。你**絕對必須**看清前面的討論，並針對前面的論點進行有理有據的逆向思考、贊同、反駁或補充！
                            4. 請用精煉且有說服力的繁體中文回答，語氣要活生生像個獨立的專家。
                            """
                            expert_avatar = {"巴菲特價值專員": "🎩", "凱薩琳科技女皇": "👑", "索羅斯總經獵手": "🦅"}.get(expert_name, "🤖")
                            with st.chat_message("assistant", avatar=expert_avatar):
